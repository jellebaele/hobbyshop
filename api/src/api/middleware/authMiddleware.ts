...
export const ensureIsAdmin = async (req: Request, res: Response, next: NextFunction) => {
...
export const checkTimeLoggedIn = async (req: Request, res: Response, next: NextFunction) => {
  if (authService.isLoggedIn(req)) {
    const now = Date.now();
